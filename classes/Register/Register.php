...
class DND_Register_Register {
	...
	public static function php_version_check() {
		if ( version_compare( phpversion(), static::php_version_required(), '<' ) ) {
			add_action( 'admin_notices', [ 'DND_Register_Register', 'unsupported_php_version' ], 10, 2 );
			return false;
		}
		return true;
	}
	...
	public static function unsupported_php_version() {
		$short = __( 'You are running an unsupported version of PHP.', 'dnd-first-edition' );
	...
	public static function wp_version_check() {
		if ( version_compare( $GLOBALS['wp_version'], static::wp_version_required(), '<' ) ) {
			add_action( 'admin_notices', [ 'DND_Register_Register', 'unsupported_wp_version' ], 10, 2 );
			return false;
		}
		return true;
	}
	...
	public static function unsupported_wp_version() {
		$short = __( 'You are running an unsupported version of WordPress.', 'dnd-first-edition' );
	...
	private static function display_admin_notice( $short, $long ) {
		...
		<div class="notice notice-error">
			<p style="max-width:800px;">
				<b><?php echo esc_html( sprintf( _x( '%s can not be activated.', 'Plugin title', 'dnd-first-edition' ), static::$title ) );?></b>
...
}  #  End of class DND_Register_Register
