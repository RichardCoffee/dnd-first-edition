...
class DND_Monster_Humanoid_Troll extends DND_Monster_Monster {
...
#	protected $to_hit_row   = array(); // DND_Monster_Trait_Combat
	...
	protected $vulnerable   = array( 'regen' );
...
#	protected $xp_value     = array( 0, 0, 0, 0 );
	protected $extra        = array( 'regen' => 0, 'nonreg' => 0 );
	...
	public function troll_regeneration( $combat, $target ) {
		if ( $this === $target ) {
			$limit = $this->current_hp + $this->extra['nonreg'];
			if ( $this->extra['regen'] && ( $limit < $this->hit_points ) ) {
				$diff = $combat->segment - $this->extra['regen'];
				if ( ( $diff > 29 ) && ( ( $diff % 10 ) === 0 ) ) {
					$delta = min( 3, ( $this->hit_points - $limit ) );
					$this->current_hp += $delta;
					if ( ( $limit + $delta ) === $this->hit_points ) {
						$this->extra['regen'] = 0;
					}
				}
			}
