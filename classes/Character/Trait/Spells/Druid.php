<?php
/*
'Tree Walk' - Pass from tree to tree
*/
trait DND_Character_Trait_Spells_Druid {


	use DND_Character_Trait_Spells_Effects_Druid;


	protected function get_druid_spell_table() {
		$level = $this->get_level( 'druid' );
		return array(
			'First' => array(
				'Animal Friendship' => array( 'page' => 'PH 55', 'type' => 'Enchantment/Charm', 'cast' => '6 turns' ),
				'Detect Magic (D)' => array( 'page' => 'PH 55,45', 'type' => 'Divination', 'cast' => '1 segment', 'duration' => '12 rounds' ),
				'Entangle' => array( 'page' => 'PH 55', 'type' => 'Alteration', 'range' => '80 feet', 'cast' => '3 segments', 'duration' => '1 turn' ),
				'Faerie Fire' => array(
					'page'      => 'PH 55',
					'type'      => 'Alteration',
					'range'     => '80 feet',
					'duration'  => sprintf( '%u rounds', $level * 4 ),
					'aoe'       => sprintf( '%u linear feet within a 40 foot radius', $level * 12 ),
					'comps'     => 'V',
					'cast'      => '3 segments',
					'saving'    => 'None',
					'special'   => sprintf( 'Party gets +2 to hit target. %u linear feet', $level * 12 ),
					'apply'     => 'druid_first_faerie_fire',
					'condition' => 'this_group_only',
					'filters'   => array(
						array( 'dnd1e_armor_class_adj', 'faerie_fire_ac_adj', 10, 2 ),
					),
					'prior'  => array( 'dnd1e_combat_init', 'faerie_fire_notice', 10, 1 ),
					'target' => 'other',
				),
				'Invisibility to Animals' => array( 'page' => 'PH 55', 'type' => 'Alteration', 'cast' => '4 segments',
					'duration' => sprintf( '%3.1u turns', ( $level * 0.1 ) + 10 ),
				),
				'Locate Animals' => array( 'page' => 'PH 56', 'type' => 'Divination', 'cast' => '1 round',
					'duration' => sprintf( '%u rounds', $level ),
					'aoe' => sprintf( '20 foot path, %u feet long', $level * 20 ),
				),
				'Magic Fang' => array( 'page' => 'spec',
					'type'      => 'Alteration',
					'range'     => 'touch',
					'duration'  => sprintf( '%u rounds', $level ),
					'aoe'       => 'Creature touched',
					'comps'     => 'V,S,M',
					'cast'      => '3 segments',
					'saving'    => 'None',
					'special'   => '+1 to hit, +1 damage',
					'condition' => 'this_target_only',
					'filters'   => array(
						array( 'dnd1e_object_to_hit_opponent', 1, 10, 3 ),
						array( 'dnd1e_weapon_damage_bonus',    1, 10, 2 ),
					),
				),
				'Predict Weather'    => array( 'page' => 'PH 56', 'type' => 'Divination', 'cast' => '1 round', 'duration' => sprintf( '%u hours', $level * 2 ) ),
				'Pass Without Trace' => array( 'page' => 'PH 56', 'type' => 'Enchantment/Charm', 'cast' => '1 round', 'duration' => sprintf( '%u turns', $level ) ),
				'Speak With Animals' => array( 'page' => 'PH 56, PH 47', 'type' => 'Alteration', 'cast' => '3 segments',
					'duration' => sprintf( '%u rounds', $level * 2 ),
				),
			),
			'Second' => array(
				'Barkskin' => array(
					'page' => 'PH 56',
					'type' => 'Alteration',
					'range' => 'Touch',
					'duration' => sprintf( '%u rounds', $level + 4 ),
					'aoe' => 'Creature touched',
					'comps' => 'V,S,M',
					'cast' => '3 segments',
					'saving' => 'None',
					'condition' => 'this_target_only',
					'filters' => array(
						array( 'dnd1e_armor_class_adj',                    1, 10, 2 ),
						array( 'dnd1e_object_Rod_saving_throws',           1, 10, 3 ),
						array( 'dnd1e_object_Staff_saving_throws',         1, 10, 3 ),
						array( 'dnd1e_object_Wand_saving_throws',          1, 10, 3 ),
						array( 'dnd1e_object_BreathWeapon_saving_throws',  1, 10, 3 ),
						array( 'dnd1e_object_Paralyzation_saving_throws',  1, 10, 3 ),
						array( 'dnd1e_object_Poison_saving_throws',        1, 10, 3 ),
						array( 'dnd1e_object_Petrification_saving_throws', 1, 10, 3 ),
						array( 'dnd1e_object_Polymorph_saving_throws',     1, 10, 3 ),
					),
					'target' => 'required',
				),
				'Charm Person or Mammal' => array( 'page' => 'PH 56', 'type' => 'Enchantment/Charm', 'cast' => '4 segments' ),
				'Create Water' => array( 'page' => 'PH 57', 'type' => 'Alteration', 'cast' => '1 turn', 'aoe' => sprintf( '%u cubic feet', $level ) ),
				'Heat Metal' => array(
					'page'       => 'PH 57',
					'type'       => 'Alteration',
					'reversible' => true,
					'range'      => '40 feet',
					'duration'   => '7 rounds',
					'aoe'        => sprintf( '%u man-sized creatures', $level ),
					'comps'      => 'V,S,M',
					'cast'       => '4 segments',
					'saving'     => 'None',
					'special'    => "Max {$level} ) Mild: 1d4 (2,6);  Severe: 2d4 (3,4,5)",
					'apply'      => 'druid_second_heat_metal',
					'filters'    => array(
						array( 'dnd1e_new_seg_enemy', 'druid_heat_metal_effect', 10, 2 ),
					),
					'status' => 'druid_heat_metal_status',
				),
				'Obscurement' => array( 'page' => 'PH 58', 'type' => 'Alteration', 'cast' => '4 segments',
					'duration' => sprintf( '%u rounds', $level * 4 ),
					'aoe'      => sprintf( '%u cubic feet', $level * 10 ),
					'special'  => 'Reduces visibility of any sort down to 2d4 feet',
				),
				'Trip' => array( 'page' => 'PH 58', 'type' => 'Enchantment/Charm', 'cast' => '4 segments',
					'duration' => sprintf( '%u turns', $level ),
					'special'  => 'Creatures running take 1d6 damage, plus stunned 1d4+1 rounds / 1d4+1 segments',
				),
				'Warp Wood' => array( 'page' => 'PH 58', 'type' => 'Alteration', 'cast' => '4 segments',
					'range' => sprintf( '%u feet', $level * 10 ),
					'aoe'   => sprintf( '%u inch diameter', $level ),
				),
			),
			'Third' => array(
				'Call Lightning' => array( 'page' => 'PH 58', 'type' => 'Alteration', 'cast' => '1 turn',
					'duration' => sprintf( '%u turns', $level ),
				),
				'Cloudburst' => array( 'page' => 'UA 43', 'type' => 'Alteration', 'cast' => '5 segments',
					'range' => sprintf( '%u feet', $level * 10 ),
				),
				'Magic Fang II' => array( 'page' => 'spec',
					'type'      => 'Alteration',
					'range'     => sprintf( '%u feet', round( $level * 2.5 ) + 25 ),
					'duration'  => sprintf( '%u rounds', $level * 2 ),
					'aoe'       => 'One targeted creature',
					'comps'     => 'V,S,M',
					'cast'      => '5 segments',
					'saving'    => 'None',
					'special'   => sprintf( '+%1$u to hit, +%1$u damage', min( 5, floor( $level / 4 ) ) ),
					'condition' => 'this_target_only',
					'filters'   => array(
						array( 'dnd1e_object_to_hit_opponent', min( 5, floor( $level / 4 ) ), 10, 3 ),
						array( 'dnd1e_weapon_damage_bonus',    min( 5, floor( $level / 4 ) ), 10, 2 ),
					),
				),
				'Plant Growth' => array( 'page' => 'PH 58-59', 'type' => 'Alteration', 'cast' => '1 round', 'aoe' => sprintf( '%1$u x %1$u square area', $level * 20 ) ),
				'Protection from Fire' => array(
					'page'      => 'PH 59',
					'type'      => 'Abjuration',
					'range'     => 'Touch',
					'duration'  => sprintf( '%u turns', $level ),
					'aoe'       => 'Creature touched',
					'comps'     => 'V,S,M',
					'cast'      => '5 segments',
					'saving'    => 'None',
					'apply'     => 'druid_third_protection_from_fire',
					'condition' => 'this_target_only',
					'effect'    => 'fire',
					'filters'   => array(
						array( 'dnd1e_absorption_hp', 12 * $level ),
						array( 'dnd1e_object_all_saving_throws', 'protection_from_fire_saving_throws', 10, 4 ),
						array( 'dnd1e_damage_to_target',         'protection_from_fire_damage',        10, 4 ),
					),
				),
				'Pyrotechnics' => array( 'page' => 'PH 59', 'type' => 'Alteration', 'cast' => '5 segments',
					'aoe' => sprintf( 'Fireworks: lasts %1$u segments, or Smoke: last %1$u rounds', $level ),
				),
				'Stone Shape'     => array( 'page' => 'PH 59, 82', 'type' => 'Alteration', 'cast' => '1 round',    'aoe' => sprintf( '%u cubic feet', $level + 3 ) ),
				'Water Breathing' => array( 'page' => 'PH 59-60',  'type' => 'Alteration', 'cast' => '3 segments', 'reversible' => true, 'duration' => sprintf( '%u turns', $level * 6 ) ),
			),
			'Fourth' => array(
				'Animal Summoning I' => array( 'page' => 'PH 60', 'type' => 'Conjuration/Summoning', 'cast' => '6 segments',
					'range' => sprintf( '%u feet', $level * 40 ),
					'aoe'   => 'Druid may attempt to summon up to three times (different animal type each time).',
				),
				'Call Woodland Beings' => array( 'page' => 'PH 60', 'type' => 'Conjuration/Summoning', 'cast' => '2 turns max.',
					'special' => 'Summoned creature gets a ST versus magic at -4',
				),
				"Control Temperature, 10' radius" => array( 'page' => 'PH 60', 'type' => 'Alteration', 'cast' => '6 segments',
					'duration' => sprintf( '%u turns', $level + 4 ),
				),
				'Magic Fang III' => array( 'page' => 'spec',
					'type'     => 'Alteration',
					'range'    => sprintf( '%u feet', round( $level * 2.5 ) + 25 ),
					'duration' => sprintf( '%u rounds', $level ),
					'aoe'      => 'One targeted creature',
					'comps'    => 'V,S,M',
					'cast'     => '6 segments',
					'saving'   => 'None',
					'special'  => sprintf( '+%1$u to hit, +%1$u damage', min( 5, floor( $level / 5 ) ) ),
					'condition' => 'this_target_only',
					'filters'   => array(
						array( 'dnd1e_object_to_hit_opponent', min( 5, floor( $level / 4 ) ), 10, 3 ),
						array( 'dnd1e_weapon_damage_bonus',    min( 5, floor( $level / 4 ) ), 10, 2 ),
						#  FIXME: May need mtdw filter as well
					),
				),
				'Speak With Plants' => array( 'page' => 'PH 61, PH 50', 'type' => 'Alteration', 'cast' => '1 turn',
					'duration' => sprintf( '%u rounds', $level * 2 ),
				),
			),
			'Fifth' => array(
				'Animal Growth' => array( 'page' => 'PH 61', 'type' => 'Alteration', 'cast' => '7 segments', 'reversible' => true,
					'duration' => sprintf( '%u rounds', $level * 2 ),
				),
				'Control Winds' => array( 'page' => 'PH 61', 'type' => 'Alteration', 'cast' => '7 segments',
					'duration' => sprintf( '%u turns', $level ),
					'aoe'      => sprintf( '%u foot radius hemisphere', $level * 40 ),
					'special'  => sprintf( 'Increase/Decrease winds by %u miles/hour', $level * 3 ),
				),
				'Tranmute Rock To Mud' => array( 'page' => 'PH 62', 'type' => 'Alteration', 'cast' => '7 segments', 'reversible' => true,
					'aoe' => sprintf( '%u cubic feet', $level * 30 ),
				),
			),
			'Sixth' => array(
				'Transport Via Plants' => array( 'page' => 'PH 63', 'type' => 'Alteration', 'cast' => '3 segments',
					'special' => sprintf( '%u%% chance of ending up 1d100 miles from desired location', max( 0, 20 - $level ) ),
				),
				'Weather Summoning' => array( 'page' => 'PH 64, PH 53', 'type' => 'Conjuration/Summoning', 'cast' => '1 turn' ),
			),
			'Seventh' => array(
				'Animate Rock' => array( 'page' => 'PH 64', 'type' => 'Alteration', 'cast' => '9 segments',
					'duration' => sprintf( '%u rounds', $level ),
					'aoe'      => sprintf( '%u cubic feet', $level * 2 ),
				),
			),
		);
	}

	protected function get_druid_description_table() {
		static $table = null;
		if ( $table ) return $table;
		$table = array(
			'First' => array(
				'Magic Fang' => 'By means of this spell, the druid is able to affect an animal that the druid has befriended.  The animal recieves a bonus of +1 to hit and +1 damage. Material component is a fang from a woodland animal.',
			),
			'Third' => array(
				'Magic Fang II' => 'This spell works identically to the first level druid spell Magic Fang, except the bonus is +1/+1 per every 4 levels of the druid, maximum of +5/+5.',
			),
			'Fourth' => array(
				'Magic Fang III' => "This spell is a more potent version of the third level druid spell Magic Fang II.  The creature's attack now counts as a magical weapon for monsters that can only be hit by magical weapons, although the bonus is +1/+1 per every five levels of the druid.",
			),
		);
		return $table;
	}


}
